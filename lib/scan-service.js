...
function buildReport(parts) {
  ...
  return {
    ...
    summary: criticalParts.length > 0
...
async function createDemoScan() {
  ...
  const scanInsert = await query(
    `
      insert into scans (status, grade, summary, total_parts, total_lines, total_quantity, total_cost, critical_count, warning_count, out_of_stock_count)
  ...
  for (const part of parts) {
    await query(
      `
        insert into parts (
          scan_id, mpn, manufacturer, description, qty, category, package, lifecycle,
          yteol, stock, unit_price, ext_price, trend, compliance, risk_level
        )
